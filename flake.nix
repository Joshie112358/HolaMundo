{
  description = "Nix flake for HolaMundo";
  ...
  outputs = { self, nixpkgs, flake-utils, haskellNix, flake-compat }:
    ...
    let
      overlays =
        [ haskellNix.overlay
          (final: prev: {
            ...
            HolaMundo = final.haskell-nix.cabalProject {
      ...
      flake = pkgs.HolaMundo.flake {};
    in flake // {
      # Built by `nix build .`
      defaultPackage = flake.packages."HolaMundo:exe:HolaMundo";

      # This is used by `nix develop .` to open a shell for use with
      # `cabal`, `hlint` and `haskell-language-server`
      devShell = pkgs.HolaMundo.shellFor {
        tools = {
          cabal = "latest";
          hlint = "latest";
